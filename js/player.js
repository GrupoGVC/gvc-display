...
async function heartbeat() {
  try {
    const res = await fetch(`${API}/devices/heartbeat.php`, {
      method:  'POST',
...
async function loadPlaylist(id) {
  try {
    const res  = await fetch(`${API}/playlists/index.php?id=${id}`, {
...
async function generateCode() {
  ...
  try {
    const res  = await fetch(`${API}/pairing/index.php?action=generate&token=${encodeURIComponent(DEVICE_TOKEN)}`);
    const raw  = await res.text();
    const data = JSON.parse(raw.replace(/^[^{[]*/, ''));
